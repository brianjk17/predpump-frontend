...
useEffect(() => {
  ...
  const checkRedeemStatus = async () => {
    ...
    try {
      ...
      const fpmm = new ethers.Contract(fpmmId || fpmmAddress, FpmmABI, provider);
      ...
      if (data) {
        ...
        if (data.push_group_id) {
          ...
          const formattedHistory = history.map(msg => ({
            fromDID: msg.fromDID,
            content: msg.messageContent || msg.messageObj?.content || msg.content,
...
const { isLoading: isConfirmingApproveReceipt, isSuccess: isConfirmedApproveReceipt } =
  useWaitForTransactionReceipt({
    hash: hashApprove,
  });
  ...
  <div className="flex flex-col items-center justify-center mt-10 w-full min-h-screen px-4 sm:px-6">
    ...
    <div className="w-full max-w-[1200px] flex flex-col justify-between gap-5">
      ...
      <div className="w-full flex-col flex justify-center items-center bg-green-800/50 p-4 sm:p-6 rounded-3xl border-4 border-dashed border-lime-400 text-white text-xl my-4 sm:my-8">
        {/* Position Selection */}
        <h1 className="text-lg sm:text-xl mt-4 press-start-2p-regular">Outcome: </h1>
        ...
        {/* Amount Input */}
        <h1 className="text-lg sm:text-xl mt-4 press-start-2p-regular">Amount: </h1>
        <input
          type="text"
          value={amount}
        ...
        <div className="w-full max-w-[600px] flex flex-col items-center">
              ...
              {isError ? (
                ...
                <div className="space-y-1">
                  <p className="text-sm sm:text-base">
                    Tokens to receive: {calculatedBuyAmount ? formatTokenAmount(calculatedBuyAmount) : "Calculating..."}
                  </p>
                  <p className="text-xs sm:text-sm text-white">
                    Price per token: {calculatedBuyAmount && amount
          ...
          <button
            className={`w-full max-w-[600px] p-3 sm:p-4 rounded text-base sm:text-xl ${
              isBuy ? "bg-teal-300 hover:bg-teal-600 text-black" : "bg-red-500 hover:bg-red-600"
            }`}
        ...
        <div className="w-full my-4 sm:my-8">
          <div className="bg-teal-300 rounded-lg p-2">
            <div className="bg-white rounded-lg p-4 sm:p-6">
              <h2 className="text-lg sm:text-xl press-start-2p-regular mb-4">Market Discussion</h2>
